 ...
 *				common_open				     *
 *===========================================================================*/
//oflags文件标志，用在文件本身已存在，指定打开该文件的方式  open系统调用时使用
//用在用在创建文件时使用,create系统调用使用
 ...
 */
    ...
    if (rip == NIL_INODE && err_code == ENOENT) {
        ...
        rw_inode(rip,
                 WRITING);        /* force inode to disk now */ //此处是否有bug rw_inode并不会立即写入磁盘，并且没写入磁盘时将i_dirt设置为clean会导致rip缓存块被淘汰
